...
import { lotteryHandler } from '@/utils/algorithm';
import { database, DB_STORE_NAME } from '@/utils/db';

export default {
  name: 'App',
  components: { LotteryConfig, Publicity, Result, Tool },
  computed: {
    resCardStyle() {
  ...
  methods: {
    ...
    toggle(form) {
      ...
      if (this.running) {
        ...
        const resArr = lotteryHandler(number, allin ? [] : this.allresult, num);
        this.resArr = resArr;
